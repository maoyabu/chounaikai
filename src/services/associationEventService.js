...
export const eventValues = body => {
  ...
  const startTime = String(body.startTime || ''), endTime = String(body.endTime || '');
  if (![startTime, endTime].every(value => /^([01]\d|2[0-3]):[0-5]\d$/.test(value))) throw fail('開始時間と終了時間を確認してください。');
  if (startDate === endDate && endTime <= startTime) throw fail('終了時間は開始時間より後にしてください。');
  const category = required(body.category, 40, '行事分類');
  const color = String(body.color || '');
  if (!/^#[0-9a-fA-F]{6}$/.test(color)) throw fail('分類カラーを選択してください。');
  return { title, body: description, startDate, endDate, startTime, endTime, category, color,
...
export const visibleEvents = async (associationIds, { publicOnly = false, now = new Date() } = {}) => {
  ...
  return AssociationEvent.find({ association: { $in: associationIds }, visible: true, ...(publicOnly ? { open: true } : {}), startDate: { $lt: end }, endDate: { $gte: start } }).sort({ startDate: 1, startTime: 1 }).lean();
};
